...
%%1. Trilateration for least-square solution.
%    But the result 'Nsol1' is not used in the following Recursive Trilateration,
%    it is calculated only for comparison (???).
%    The reason is that RecTrilateration(np>3) includes the recursive call of 
%    RecTrilateration(np==3), which is only slight different (whether there is 
%    difference is to be confirmed) from Trilateration(np==3).
fprintf('\n ---Trilateration: Direct least-square solution --- \n');
S = S38;
...
%%2. Recursive Trilateration for least-square solution.
fprintf('\n ---Trilateration: Recursive least square--- \n');
Nmat0 = RecTrilateration(P,S,W);
Nmat  = Nmat0(:,1:5); % each column of Nmat represents a solution.
fprintf('\n Recursive solutions \n');
disp(Nmat(2:4,:))
%[n_mat m_mat]=size(Nmat);
for i1=1:size(Nmat,2)
    Nn = Nmat(2:4,i1);
    %Nn = Nn(2:4);
